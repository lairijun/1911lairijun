...
class titlebarList extends Component{
  constructor(){
      super()
      this.state={
        dataSource:[],
...
confirmDel=(_id)=>{
  let url='/hehe/admin/titlebar/titlebardel'
  let data={_id:_id}
  this.$axios.post(url,data).then((res)=>{
    ...
    initData(page,pageSize){
      let url='/hehe/admin/titlebar/titlebarlist';
  ...
  return(
      <Card className='userInfo'>
          ...
          <Spin tip='数据加载中' spinning={loading}>
               {!updateShow||<TitlebarUpdate record={record} refreshfun={this.refresh} ></TitlebarUpdate>}
